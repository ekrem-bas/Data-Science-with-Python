...
categorical_cols = [col for col in df.columns if str(df[col].dtypes) in ["object", "bool", "category"]]
# numerical but also categorical columns
num_but_categorical = [col for col in df.columns if df[col].nunique() < 10 and df[col].dtypes in ["int", "float"]]
# it seems categorical but there is to many values that they do not categorical columns
categorical_but_cardinal = [col for col in df.columns if df[col].nunique() > 20 and str(df[col].dtypes) in ["category", "object"]]
# in the end categorical columns are
categorical_cols = categorical_cols + num_but_categorical
# if there are categorical but cardinal columns, categorical columns are
categorical_cols = [col for col in categorical_cols if col not in categorical_but_cardinal]

df[categorical_cols].nunique() # all of them lover than 10

[col for col in df.columns if col not in categorical_cols] # numerical but not categorical columns
...
def grab_col_names(dataframe, cat_th=10,  car_th=20):
    ...
    ----------
    dataframe: DataFrame
        The DataFrame from which variable names will be retrieved.
...
high_correlated_cols(cor_matrix)
drop_list = high_correlated_cols(cor_matrix, plot=True)
df.drop(drop_list, axis=1)
high_correlated_cols(cor_matrix.drop(drop_list, axis=1), plot=True)

notlar = [68, 74, 82, 90, 78, 85, 92, 88, 76, 61, 79, 73, 89, 81, 72, 95, 70, 83, 77, 75]
plt.hist(notlar, bins=5, edgecolor='r', alpha=0.7)
plt.xlabel('Notlar')
plt.ylabel('Frekans')
